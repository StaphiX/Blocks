...
public static class DebugExtension
{
	...
	public static void DrawBounds(Bounds bounds, Color color)
	{
		...
		Debug.DrawLine(ruf, luf, color);
		Debug.DrawLine(ruf, rub, color);
		Debug.DrawLine(luf, lub, color);
		Debug.DrawLine(rub, lub, color);
		
		Debug.DrawLine(ruf, rdf, color);
		Debug.DrawLine(rub, rdb, color);
		Debug.DrawLine(luf, lfd, color);
		Debug.DrawLine(lub, lbd, color);
		
		Debug.DrawLine(rdf, lfd, color);
		Debug.DrawLine(rdf, rdb, color);
		Debug.DrawLine(lfd, lbd, color);
		Debug.DrawLine(lbd, rdb, color);
	}
	...
	public static void DrawCircle(Vector3 position, Vector3 up, Color color, float radius = 1.0f)
	{
		...
		for(var i = 0; i < 91; i++){
			...
			Debug.DrawLine(_lastPoint, _nextPoint, color, 0.0f, true);
			_lastPoint = _nextPoint;
		}
	}
	...
	public static void DrawCylinder(Vector3 start, Vector3 end, Color color, float radius = 1.0f){
		...
		//Start endcap
		Debug.DrawLine(start-right, start+right, color);
		Debug.DrawLine(start-forward, start+forward, color);
		
		//End endcap
		Debug.DrawLine(end-right, end+right, color);
		Debug.DrawLine(end-forward, end+forward, color);
	}
	...
	public static void DrawCone(Vector3 position, Vector3 direction, Color color, float angle = 45)
	{
		...
		Debug.DrawRay(position, slerpedVector.normalized*dist, color, 0.0f, true);
		Debug.DrawRay(position, Vector3.Slerp(_forward, -_up, angle/90.0f).normalized*dist, color, 0.0f, true);
		Debug.DrawRay(position, Vector3.Slerp(_forward, _right, angle/90.0f).normalized*dist, color, 0.0f, true);
		Debug.DrawRay(position, Vector3.Slerp(_forward, -_right, angle/90.0f).normalized*dist, color, 0.0f, true);
	...
	{
		Debug.DrawRay(position, direction, color, 0.0f, true);
